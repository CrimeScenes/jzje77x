...
local lastShotTime = 0  -- To prevent multiple shots at once
local accuracyThreshold = 0.9  -- Minimum accuracy to fire (90%)

-- Function to simulate mouse click event
...
-- Function to check if a position is within viewable area
...
-- Function to get body parts positions of a character
...
-- Function to check if we're ignoring knives or katana tools
...
-- Function to check if mouse is on target
...
-- Function to handle TriggerBot action
...
local function refinePrediction(BodyPart)
    local humanoid = TargetPlayer.Character:FindFirstChildOfClass("Humanoid")
    if humanoid then
        -- Improved prediction: Calculate velocity and compensate for future movement
        local targetVelocity = humanoid.MoveDirection * humanoid.WalkSpeed
        local predictedPosition = BodyPart.Position + (targetVelocity * Vector3.new(
        ...
        -- Check the distance to predicted position
        return predictedPosition
    end
    return BodyPart.Position  -- If no humanoid or move direction, fallback to current position
end
...
-- Track mouse button hold for TriggerBot
UserInputService.InputBegan:Connect(function(input, gameProcessed)
...
RunService.RenderStepped:Connect(function()
    if IsTargeting and TargetPlayer and TargetPlayer.Character then
        ...
        if BodyPart then
            ...
            local predictedPosition = refinePrediction(BodyPart)

            -- Apply stabilization to prediction
            ...
            if shared.Forbidden.Silent.Enabled and IsTargeting and TargetPlayer.Character:FindFirstChild("Humanoid") then
                local closestPoint = predictedPosition  -- Use the refined predicted position
                local velocity = GetVelocity(TargetPlayer, BodyPart.Name)

                -- Calculate accuracy threshold (90%)
                local screenPos, onScreen = Camera:WorldToViewportPoint(closestPoint)
                local screenCenter = Vector2.new(Camera.ViewportSize.X / 2, Camera.ViewportSize.Y / 2)
                local distanceToCenter = (Vector2.new(screenPos.X, screenPos.Y) - screenCenter).Magnitude
                local accuracy = distanceToCenter / Camera.ViewportSize.X

                -- Only fire if accuracy is within the threshold
                if accuracy <= 0.1 then  -- 0.1 means 90% accuracy (closer to center)
                    ...
                    -- TriggerBot Integration: Only trigger if mouse is held
